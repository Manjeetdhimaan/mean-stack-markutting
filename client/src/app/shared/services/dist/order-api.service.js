"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
exports.__esModule = true;
exports.OrderApiService = void 0;
var core_1 = require("@angular/core");
var environment_1 = require("src/environments/environment");
var OrderApiService = /** @class */ (function () {
    function OrderApiService(http) {
        this.http = http;
    }
    OrderApiService.prototype.postPlaceOrder = function (orderBody, domain) {
        var order = Object.assign({}, { orderBody: orderBody, domain: domain });
        return this.http.post(environment_1.environment.apiBaseUrl + '/orders/post-order', order);
    };
    OrderApiService.prototype.postOrderResponse = function (orderSessionId) {
        return this.http.post(environment_1.environment.apiBaseUrl + '/orders/post-order-response', { orderSessionId: orderSessionId });
    };
    OrderApiService.prototype.getUserOrders = function () {
        return this.http.get(environment_1.environment.apiBaseUrl + '/orders/get-user-orders');
    };
    OrderApiService.prototype.getAllOrders = function () {
        return this.http.get(environment_1.environment.apiBaseUrl + '/orders/get-orders');
    };
    OrderApiService.prototype.getUserOrder = function (orderId) {
        return this.http.get(environment_1.environment.apiBaseUrl + '/orders/get-order/' + orderId);
    };
    OrderApiService = __decorate([
        core_1.Injectable({
            providedIn: 'root'
        })
    ], OrderApiService);
    return OrderApiService;
}());
exports.OrderApiService = OrderApiService;
